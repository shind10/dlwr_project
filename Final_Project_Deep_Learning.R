...
train_data <- read_csv("~/Desktop/deep learning in R/Deeplearning-project/train.csv")%>% select(-label) ## or we can manually do this like, train_data$lable=NULL ##
train_labels <- read_csv("~/Desktop/deep learning in R/Deeplearning-project/train.csv") %>% select(label)
test_data <- read_csv("~/Desktop/deep learning in R/Deeplearning-project/test.csv") %>% select(-id) ## or we can manually do this like, test_data$id=NULL ##
val_data <- read_csv("~/Desktop/deep learning in R/Deeplearning-project/Dig-MNIST.csv") %>% select(-label)
val_labels <- read_csv("~/Desktop/deep learning in R/Deeplearning-project/Dig-MNIST.csv") %>% select(label)
...
batch_size <- 128
num_classes <- 10 
epochs <- 25 # we can change this to get faster result # 
